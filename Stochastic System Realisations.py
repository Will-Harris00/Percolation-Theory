...
def main():
    ...
    nrep = [1e1,50,1e2,500,1e3,5000,1e4]
    ...
    for N in sizes:
        ...
        plt.show()
    with pd.ExcelWriter("percolationpcnrep.xlsx") as writer:
        writer.save()
        print("Created the excel file 'percolationpcnrep.xlsx'")
    try:
        with pd.ExcelWriter("percolationpcnrep.xlsx", engine='openpyxl', mode='a') as writer:
            writer.book = load_workbook("percolationpcnrep.xlsx")
        ...
        print("Please ensure the file 'percolationpcnrep.xlsx' is not open in another program")
