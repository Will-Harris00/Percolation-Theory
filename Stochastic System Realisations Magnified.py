...
def main():
    ...
    step = 0.005
    pc = pd.DataFrame(columns=["Realisations", "Crit_Perc"])
    for N in sizes:
        excel_file = "percolationz" + str(N) + ".xlsx"
        ...
        for i in nrep:
            ...
            plt.xticks(np.arange(0.45, 0.55+0.001, step=0.01))
            plt.yticks(np.arange(0, 0.0015+0.0001, step=0.00025))
            plt.show()
            pc.loc[nrep.index(i)] = [i, critperc]
            print(pc)
        pc.plot(x='Crit_Perc', y='Realisations', kind='scatter')
        plt.title("Critical Percolation against Number of Realisations \nMatrix Size " + str(N) + "x" + str(N))
        plt.xlim(0, 1)
        plt.ylim(0, i)
        ...
        plt.show()
    with pd.ExcelWriter("percolationpczoom.xlsx") as writer:
        writer.save()
        print("Created the excel file 'percolationpczoom.xlsx'")
    try:
        with pd.ExcelWriter("percolationpczoom.xlsx", engine='openpyxl', mode='a') as writer:
            writer.book = load_workbook("percolationpczoom.xlsx")
            sheetName = "Crit Perc Varying Realisations"
        ...
        print("Please ensure the file 'percolationspczoom.xlsx' is not open in another program")
