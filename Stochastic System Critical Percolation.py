...
def main():
    ...
    step = 0.01
    pc = pd.DataFrame(columns=["Size", "Crit_Perc"])
    for N in sizes:
        ...
        for i in nrep:
            ...
            plt.show()
        pc.loc[sizes.index(N)] = [N, critperc]
        print(pc)
    pc.plot(x='Size', y='Crit_Perc', kind='scatter')
    plt.title("Critical Percolation against Matrix Size \nNumber of Realisations " + str(i))
    plt.xlim(0, 1)
    plt.ylim(0, N)
    plt.xlabel('Critical Percolation')
    plt.ylabel('Size of matrix')
    plt.xticks(np.arange(0, 1+0.1, step=0.1))
    plt.yticks(np.arange(0, N+0.1, step=50))
    plt.show()
    with pd.ExcelWriter("percolationpcsize.xlsx") as writer:
        writer.save()
        print("Created the excel file 'percolationpcsize.xlsx'")
    try:
        with pd.ExcelWriter("percolationpcsiza.xlsx", engine='openpyxl', mode='a') as writer:
            writer.book = load_workbook("percolationpcsize.xlsx")
            sheetName = "Crit Perc Varying Sizes"
        ...
        print("Please ensure the file 'percolationpcsize.xlsx' is not open in another program")
