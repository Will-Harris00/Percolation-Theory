...
def simulation(p, N, nrep, animate, separate):
    if separate == True:
        # empties animation array after change in density of rock in the sand.
        global ims
        ims = []
    ...
    for j in range(int(nrep)):
        ...
        while r < N - 1:
            ...
            ## Exception for attempting to index outside of bounds
            except IndexError:
                break
...
def main():
    # Would you like to animate the simulations, please note this may take a while
    animate = True
    ...
    step = 0.1
    pc = pd.DataFrame(columns=["Grid Size", "Realisations", "Crit_Perc"])
    k = 0
    for N in sizes:
        ...
        for i in nrep:
            ...
            df = pd.DataFrame(columns=["Density", "Number_Bottom",
                                       "Frequency_Reach_Bottom", "Total_Depth",
                                       "Average_Depth"])
            j = 0
            while p > 0:
                p = round(p, 2)
            ...
            try:
                ...
                print("Please ensure the file '" + str(excel_file) +
                      "' is not open in another program")

            print(df)
            df.plot(x='Density', y='Frequency_Reach_Bottom', kind='scatter')
            plt.title("Water Percolation - " + str(i) + " Realisations - " +
                      str(N) + "x" + str(N) + " Grid")
            plt.xlim(0, 1)
            plt.ylim(0, 1)
    ...
    sns.scatterplot(data=pc, x="Crit_Perc", y="Realisations", hue='Grid Size',
                    legend='full', markers=["o", "x", "1", "s", "d"],
                    palette=['green', 'orange', 'purple', 'dodgerblue', 'red'])
    plt.title("Critical Percolation against Number of Realisations")
    plt.xlim(0, 1)
    plt.ylim(0, i)
    plt.xlabel('Critical Percolation')
    plt.ylabel('Number of Realisations')
    plt.legend(loc='upper left')
    plt.xticks(np.arange(0, 1+0.1, step=0.1))
    plt.yticks(np.arange(0, i+0.1, step=max(nrep)/10))
    plt.show()
    with pd.ExcelWriter("percolationpcall.xlsx") as writer:
        writer.save()
        print("Created the excel file 'percolationpcall.xlsx'")
    try:
        with pd.ExcelWriter("percolationpcall.xlsx", engine='openpyxl', mode='a')\
                as writer:
            writer.book = load_workbook("percolationpcall.xlsx")
            sheetName = "Critical Percolations All"
        ...
        print("Please ensure the file 'percolationpcall.xlsx' "
              "is not open in another program")
