...
def simulation(p, ny, nx, nrep, animate, separate):
    if separate == True:
        # empties animation array after change in density of mud in the forest.
        global ims
        ims = []
...
def main():
    ...
    for n in sizes:
        ...
        for i in nrep:
            ...
            plt.title("Forest Fire Percolation - " + str(i) + " Realisations - "
                      + str(n) + "x" + str(n) + " Grid")
            plt.xlim(0, 1)
            plt.ylim(0, 1)
    ...
    sns.scatterplot(data=pc, x="Crit_Perc", y="Realisations", hue='Grid Size',
                    ...
                    palette=['Burlywood', 'DarkSlateGray',
                             'SaddleBrown', 'Teal', 'Olive'])
    ...
    plt.show()
    with pd.ExcelWriter("forestpcall.xlsx") as writer:
        writer.save()
        print("Created the excel file 'forestpcall.xlsx'")

    try:
        with pd.ExcelWriter("forestpcall.xlsx", engine='openpyxl', mode='a') \
                as writer:
            writer.book = load_workbook("forestpcall.xlsx")
        ...
        print("Please ensure the file 'forestpcall.xlsx' "
              "is not open in another program")
